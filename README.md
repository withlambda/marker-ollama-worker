...
2.  **vLLM Phase**: After MinerU completes, CUDA cache is cleared and a configurable VRAM recovery delay (`MARKLLM_VLLM_VRAM_RECOVERY_DELAY`) ensures GPU memory is fully released before vLLM starts.
...
1.  **VRAM Logs**: Look for `VRAM Usage (After MinerU)` in the worker logs. If `Free` memory is low (e.g., < 2GB) before vLLM starts, increase `MARKLLM_VLLM_VRAM_RECOVERY_DELAY` or reduce `MARKLLM_VLLM_GPU_UTIL`.
...
1.  Clone the repository:
    ```bash
    git clone https://github.com/your-username/markllm-mineru-worker.git
    cd markllm-mineru-worker
    ```

2.  Build the Docker image:
    ```bash
    docker build -t markllm-mineru .
    ```
...
1.  **Create a Template**: In RunPod, create a new Serverless Template.
    *   **Image Name**: Your pushed image (e.g., `ghcr.io/your-username/markllm-mineru-worker:latest`).
...
| `MARKLLM_VLLM_VRAM_RECOVERY_DELAY`         | Seconds to wait after MinerU before starting vLLM.                                                                                                                                                                        | `10`                                          |
...
| `MARKLLM_VLLM_CHUNK_SIZE`                  | Size of text chunks in tokens for correction phase; effective chunk size is reduced by prompt-token usage/context safety margin; oversized blocks are split into non-overlapping chunks using `langchain-text-splitters`. | `4000`                                        |
...
| `MARKLLM_VLLM_VRAM_RECOVERY_DELAY` | vLLM              | Seconds to wait after MinerU before starting vLLM.                                              | `10`                                                                    |
...
- The `MINERU_WORKERS` and `MARKLLM_VLLM_MAX_NUM_SEQS` values are auto-calculated based on available VRAM when set to `auto`.
